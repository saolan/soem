...
//	public DimmPlanCuen buscarDimmPlanCuenIva() {
//
...
//
//		dimm.setDimmId(13030);
//
//		dimmPlanCuenBuscar.setPlanCuen(new PlanCuen());
//		dimmPlanCuenBuscar.setDimm(dimm);
//
//		try {
...
//
//				if (dimmPlanCuen.getTipoTran().equals("COMPRA")) {
...
//			Modifica la nota del ingreso para pasar eso a la nota de la tansaccion
			...
			ingreso.setNota(ingreso.getNota() + " " + ingreso.getSerie1() + "-" + ingreso.getSerie2() + "-"
					...
					+ simpleDateFormat.format(ingreso.getFechaEmis()));
...
//			Revisar que la nota no tenga valores nulos
			if (formPagoMoviIngr.getNota() == null) {
				formPagoMoviIngr.setNota(formPagoMoviIngr.getDocuMoviIngr().getDocumento().getDescri() + " "
						...
						+ simpleDateFormat.format(formPagoMoviIngr.getFecha()));
			} else {
...
//					Fin
	...
	public void calcularDiasPlaz() {
		...
		// Se cambia el formato de las fechas para que calcule bien los dias caso
		// contrario toma en cuenta horas minutos y segundos
		Date fechaPago = Util.cambiarFormatoFecha(this.fechaPago, "dd-MM-yyyy");
		Date fechaEmis = Util.cambiarFormatoFecha(this.ingreso.getFechaEmis(), "dd-MM-yyyy");
//		Date fechaRegi = Util.cambiarFormatoFecha(this.ingreso.getFechaRegi(), "dd-MM-yyyy");

//		if (fechaPago.equals(fechaRegi)) {
//			this.ingreso.setDiasPlaz((short) 0);
//		} else {
		this.ingreso.setDiasPlaz((short) ((fechaPago.getTime() - fechaEmis.getTime()) / 86400000));
//		}
	}

	public void calcularFechaPago() {

//		if (this.ingreso.getDiasPlaz() == 0) {
...
//		} else {
		...
		calendar.setTime(this.ingreso.getFechaEmis());

		if (this.ingreso.getDiasPlaz() == 30 && this.ingreso.getDiasPlaz() == 31) {
			calendar.add(Calendar.MONTH, 1);
		} else {
			calendar.add(Calendar.DATE, this.ingreso.getDiasPlaz());
		}

		this.fechaPago = calendar.getTime();
//		}
	}
...
//					para que permita desprocesar
			...
			ingreso.setFechaEmis(new Date());
			ingreso.setFechaRegi(new Date());
...
//		this.persClieRegis.setDimm(new Dimm());
		...
		if (this.persProvRegis.getPersonaId() == null) {
			...
			this.persProvRegis.setFechaAuto(new Date());
...
//				Generar Liquidacion de compra electronica
				if (this.getIngreso().getDocuIngr().getDocumeElec().equals("Liquidacion")) {
					...
					if (this.variablesSesion.isFactElecCompraAuto() == true) {
...
//		Contabiliza Compra
		if (this.getIngreso().getDocuIngr().getDocumento().isContab()) {
			...
			transaccionId = this.contabilizarCompra(this.ingreso);
...
//			la transaccion
			if (this.formPagoMoviIngr.getDocuMoviIngr().getDocumento().isContab()) {
				...
				if (this.formPagoMoviIngr.getFormPago().getTipo().equals("FP")
					...
					transaccionId = this.contabilizarPago(formPagoMoviIngr);
				}
			}
		}
	...
	public String grabarIngreso(String estado) {
		...
		try {
			...
			// Graba el total de la factura
//			TotalDocu totalDocu = new TotalDocu();
//			totalDocu.setDescri("Total Documento");
//			int indice = this.totalDocus.indexOf(totalDocu);
//			ingreso.setTotal(totalDocus.get(indice).getValor());
...
//					Fin Genera numeración automatica si liquidación
					...
					String fechaEmision = Util.cambiarFormatoFechaString(this.ingreso.getFechaRegi(), "ddMMyyyy");
					...
					String claveAcceso = documeElectRegis.generarClaveAcceso(this.getVariablesSesion().getSucursal(),
							fechaEmision, this.getIngreso().getDocuIngr().getDimm().getCodigo(), ingreso.getSerie1(),
...
//		Buscar Iva del producto, si tiene iva se crea ingrDetaImpu sino no se crea ingrDetaImpu
		...
		for (ProdDimm prodDimmRecorrer : prodDimms) {
			if (prodDimmRecorrer.getDimm().getDimmId().equals(13030)
					|| prodDimmRecorrer.getDimm().getDimmId().equals(13040)) {
...
//				El producto es un gasto
	...
	public void agregarProducto() {
		...
		ingrDeta.setProducto(prodPrecSele.getProducto());
		ingrDeta.setPrecio(prodPrecSele.getPrecio());
		ingrDeta.setFactor(prodPrecSele.getFactor());
		...
		ingrDeta.setFechaRegi(new Date());

//		Agrega precio de vena al publico como referencia (Erotik)
...
//	y a la lista de detalle de productos (egreDetaDataTable)
	...
	public Dimm buscarIva() {
		...
		try {
			...
			dimmIva = compra.buscarDimmPorId(13030);
...
//						El ultimo código 11100 es el ice de fundas plasticas
	...
	public Set<IngrDetaPrec> crearIngrDetaPrec(IngrDeta ingrDeta) {
		...
		try {
			prodPrecs = compra.filtrarProdPrecs(prodPrecs, persUsuaSesion, variablesSesion.getRolPrecs(),
					variablesSesion.getSucursal());
		} catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, null,
	...
	public void crearKardex() {
		...
		for (IngrDeta ingrDeta : this.ingrDetaDataTable) {
			...
			Kardex kardex = new Kardex();
			kardex.setSucursal(variablesSesion.getSucursal());
			...
			kardex.setEgreDeta(null);
			kardex.setDocumeFech(ingrDeta.getIngreso().getFechaRegi());
			kardex.setDocumeNume(ingreso.getNumero());
...
//	tambien se usa ek metodo this.calcularPrecio(this.ingrDetaSele)
	...
	public Retencion crearRetencion() {
		...
		if (this.getIngreso().getDocuIngr().getTipoRete().equals("Manual")) {
			...
			try {
				fechaAutorizacion = simpleDateFormat.parse(parametro.getDescri());
			} catch (ParseException e) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_FATAL, null, "Excepción - Error transformar fecha"));
				e.printStackTrace();
			}
			...
			String claveAcce = documeElectRegis.generarClaveAcceso(this.getVariablesSesion().getSucursal(),
					fechaEmision, this.getIngreso().getDocuIngr().getCodigoTipoCompRete(), retencion.getSerie1(),
					retencion.getSerie2(), numero, this.getIngreso().getDocuIngr().getAmbien(),
					this.getIngreso().getDocuIngr().getTipoEmis(), variablesSesion.getCodiClav());
			// Fin Generar clave acceso

			retencion.setAutori(claveAcce);
			retencion.setClaveAcce(claveAcce);
			// retencion.setFechaAuto(new Date());
...
//		Primefaces 5
//		context.addCallbackParam("procesar", true);
	...
	public String convertir() {
		...
		for (IngrDeta ingrDeta : this.ingrDetaDataTable) {
			...
			// Graba ingrDetaImpu del documento origen
			// Ya no se usa porque esto se grabar al seleccionar al proveedor
			// for (IngrDetaImpu ingrDetaImpu : ingrDeta.getIngrDetaImpus()) {
			// ingrDetaImpu.setIngrDetaImpuId(null);
			// ingrDetaImpu.setIngrDeta(ingrDeta);
			// }
		...
		ingresoDuplicado.setFechaRegi(new Date());
		ingresoDuplicado.setFechaEmis(new Date());
...
//	}
	...
	public String procesarComprobanteElectronico() {
		...
		if (estadoProceso == "GENERADO") {
			...
			estadoProceso = firmarArchivo(this.getVariablesSesion().getSucursal(), rutaGenerados,
					this.variablesSesion.getRutaFirmados());
	...
	public String crearArchivoLiquidacionXml() {
		...
		try {
			...
			object = documeElectRegis.generarComprobanteElectronicoLiquidacion(this.ingreso, this.ingrDetaDataTable,
					this.formPagoMoviIngr, variablesSesion.getCodigoReteIva(),
					variablesSesion.getCodigoReteRenta(), variablesSesion.getCodigoReteIsd(),
					variablesSesion.getCodigoIva(), variablesSesion.getCodigoIce(), variablesSesion.getCodigoIrbpnr());

		} catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, null, "Excepción - Error al generar comprobante - Liquidación de Compra"));
			e.printStackTrace();
		}
	...
	public String firmarArchivo(Sucursal emisor, String archivoACrear, String rutaFirmados) {

		// Esta valiable venia como parametro. Se usa solo para que entre a firmar
		// Actualizar al implemetar firmas para linx y mac
		String password = null;
		String respuestaFirma = "GENERADO";

		try {

			String tokenId = variablesSesion.getToken();

			if ((password == null) || (System.getProperty("os.name").toUpperCase().indexOf("LINUX") == 0)
					|| (System.getProperty("os.name").toUpperCase().indexOf("MAC") == 0)) {

				respuestaFirma = x509Utils.firmaValidaArchivo(new File(archivoACrear), rutaFirmados, emisor.getRuc(),
						tokenId, password, variablesSesion.getVariosCertificados());

			}
			// else {
			// TODO: FIRMAR CON OTRA LIBRERIA XaDesBes.jar; incluir o copiar clases
			// respuestaFirma = ServicioFirmaXades.firmaValidaArchivo(new
			// File(archivoACrear), dirFirmados, emisor.getRuc(), password.toCharArray());
			// }
