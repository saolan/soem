...
//			Revisar que la nota no tenga valores nulos
			if (formPagoMoviIngr.getNota() == null) {
				formPagoMoviIngr.setNota(formPagoMoviIngr.getDocuMoviIngr().getDocumento().getDescri() + " " + formPagoMoviIngr.getFormPago().getDescri() + " " + formPagoMoviIngr.getNumero() + " " + 
						formPagoMoviIngr.getPersona().getApelli() + " Ref:" + formPagoMoviIngr.getRefere() + " " +  simpleDateFormat.format(formPagoMoviIngr.getFecha()) );				
			} else {
...
//	Se paso al EJB formPagoLista
//	public void filtrarFormPagos () {
//		
//		List<FormPago> formPagos = new ArrayList<>(this.formPagoTmps);
//		
//		this.formPagos.clear();
////		this.formPagoMoviIngr.setPersona(new Persona());
//		this.cxpSeles.clear();
//		this.cxps.clear();
// 
//		try {
//			formPagos = formPagoLista.filtrarFormPagos(formPagos, this.persUsuaSesion, variablesSesion.getRolFormPagos());
//		} catch (Exception e) {
//			FacesContext.getCurrentInstance().addMessage(null,
...
//					this.formPagos.add(formPago);
//				}
//			} else if (this.formPagoMoviIngr.getDocuMoviIngr().getTipo().equals("DEPOSITO")) {
...
//			solo sirve de referencia para saber que transaccion se hace en el caso que sea otro pago diferente de CXP
			...
			// this.formPagoMoviIngr = new FormPagoMoviIngr();
			this.getFormPagoMoviIngr().setFecha(new Date());
