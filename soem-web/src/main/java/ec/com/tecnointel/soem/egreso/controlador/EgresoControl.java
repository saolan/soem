...
//				Solo puede ver las facturas de su sucursal y de la caja abierta
	...
	public Transaccion buscarTransaccionPorId(Integer transaccionId) {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, null, "Excepcion - Error al buscar transacción por Id"));
			e.printStackTrace();
		}
...
//			Asignar transaccion con el tipo de documento
			for (EgreTran egreTran : this.egreTrans) {
				Transaccion transaccion = this
						.buscarTransaccionPorId(egreTran.getTransaccion().getTransaccionId());
	...
	public CajaMovi buscarSesionVentas() {
		...
		try {
			...
			if (cajaMovis.size() == 0){
				...
				sesionVentas = false;
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, null, "Se ha registrado mas de una sesión de ventas"));
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, null, "Excepcion - Error al buscar movimientos de caja"));
			e.printStackTrace();
...
//		En esta ruta se compila el reporte dentro del servidor de aplicaciones dentro del archivo .EAR .WEB
	...
	public void crearArchivo(Map<String, Object> parametrosJasper, String rutaJrxml, String rutaReporteCompilado, String formato, SaliArch saliArch) {
...
//			Buscar variables iva para poder separar los subtotales e imprimir en la factura los valores por separado
			...
			if (saliArch.getSalida().equals("IMPRESORA")){
				...
				jrPrintServiceExporter.exportReport();
					...
					generaJasperReportes.abrirReporte(byteArrayOutputStream.toByteArray(), nombreReporte, "application/pdf");
				...
				if (this.egresoSele.getPersClie().getPersona().getCorreo() != null 
						...
						this.ejecutarTareaEnviarCorreoDocu(this.egresoSele.getPersClie().getPersona().getCorreo(), this.egresoSele.getClaveAcce());	
				}
			}
	...
	public void ejecutarTareaEnviarCorreoDocu(String destinatario, String claveAcce) {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, null, "Excepcion - Documento no enviado por correo al cliente " + destinatario));
			e.printStackTrace();
		}
	}
