...
public class TransaccionListaImp extends GestorListaSoem<Transaccion> implements TransaccionListaInt, Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Transaccion> transaccionRoot, Transaccion transaccion) {
		...
		Date fechaEmis = transaccion.getFechaEmis();
		if (fechaEmis != null) {
			predicates.add(builder.greaterThanOrEqualTo(transaccionRoot.<Date> get("fechaEmis"), fechaEmis));
		}
	...
	@Override
	public void imprimir(Transaccion transaccion) throws Exception {
	}
