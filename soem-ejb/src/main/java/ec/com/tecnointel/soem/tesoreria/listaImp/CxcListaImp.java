...
public class CxcListaImp extends GestorListaSoem<Cxc> implements CxcListaInt, Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Cxc> cxcRoot, Cxc cxc) {
		...
		Date fechaEmis = cxc.getEgreso().getFechaEmis();
		if (fechaEmis != null) {
			predicates.add(builder.greaterThanOrEqualTo(cxcRoot.get("egreso").<Date> get("fechaEmis"), fechaEmis));
		}

		Date fechaVenc = cxc.getFechaVenc();
		if (fechaVenc != null) {
			predicates.add(builder.greaterThanOrEqualTo(cxcRoot.<Date> get("fechaVenc"), fechaVenc));
		}
