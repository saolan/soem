...
public class CxpListaImp extends GestorListaSoem<Cxp> implements CxpListaInt, Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Cxp> cxpRoot, Cxp cxp) {
		...
		Date fechaEmis = cxp.getIngreso().getFechaEmis();
		if (fechaEmis != null) {
			predicates.add(builder.greaterThanOrEqualTo(cxpRoot.get("ingreso").<Date> get("fechaEmis"), fechaEmis));
		}

		Date fechaVenc = cxp.getFechaVenc();
		if (fechaVenc != null) {
			predicates.add(builder.greaterThanOrEqualTo(cxpRoot.<Date> get("fechaVenc"), fechaVenc));
		}
